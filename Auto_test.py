...
def writeBug(bug_id, interface_name, request, response, res_check):
    ...
    # 来判断提bug的严重程度和提交给谁
    sql = "INSERT INTO `t_info` (`created_at`, `created_by`, `updated_at`, `updated_by`, `bug_status`, `assign_to`, `title`, `mail_to`, `repeat_step`, `lock_version`, `resolved_at`, `resolved_by`, `closed_at`, `closed_by`, `related_bug`, `related_case`, `related_result`, `productmodule_id`, `modified_by`, `solution`, `duplicate_id`, `product_id`, `reopen_count`, `priority`, `severity`) VALUES ('%s', '1', '%s', '1', 'Active', '1', '%s', '系统管理员', '%s', '1', NULL , NULL, NULL, NULL, '', '', '', NULL, '1', NULL, NULL, '1', '0', '1', '1');" % (now, now, bug_title, step)
    # 建立连接，使用MMySQLdb模块的connect方法连接mysql，传入账号、密码、数据库、端口、ip和字符集
    coon = MySQLdb.connect(user='admin', passwd='Admin_12345', db='auto_test', port=3306, host='10.0.6.55', charset='utf8')
    # 建立游标
    cursor = coon.cursor()
    # 执行sql
    cursor.execute(sql)
